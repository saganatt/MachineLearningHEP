...
def vardistplot(dfs_input_, mylistvariables_, output_,
    ...
    for ind, var in enumerate(mylistvariables_):
        ...
        for label, color in zip(dfs_train, colors):
...
def vardistplot_probscan(dataframe_, mylistvariables_, modelname_, thresharray_, # pylint: disable=too-many-statements
    ...
    xrange_max = []
    ref_hists = []
    get_logger().info("dataframe:")
    get_logger().info(dataframe_)
...
def scatterplot(dfs_input_, mylistvariablesx_,
    ...
    for ind in range(len(mylistvariablesx_)):
        axcorr = plt.subplot(3, int(len(mylistvariablesx_)/3+1), ind)
        plt.xlabel(mylistvariablesx_[ind], fontsize=11)
        plt.ylabel(mylistvariablesy_[ind], fontsize=11)
        title_str = 'Pearson coef. '
        for label, color in zip(dfs_train, colors):
            plt.scatter(
                dfs_input_[label][mylistvariablesx_[ind]], dfs_input_[label][mylistvariablesy_[ind]],
                alpha=0.4, c=color, label=label)
            pearson = dfs_input_[label].corr().loc[mylistvariablesx_[ind]][mylistvariablesy_[ind]].round(2)
