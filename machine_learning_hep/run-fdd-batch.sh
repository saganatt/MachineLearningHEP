...
for fd in `seq 0 10 90` ; do
  ...
  RESDIR="/data2/mkabus/MLHEP/results-multiclass-fd_${fd}-bkg_rv/"
  rm -rf "${RESDIR}"
  
  CUR_DB="${DATABASE}_edit_fd${fd}.yml"
  cp "${DATABASE_EXT}" "${CUR_DB}" || ErrExit "Could not copy database"

  sed -i "s/results-multiclass-fd_.*-bkg_rv/results-multiclass-fd_${fd}-bkg_rv/g" "${CUR_DB}" || ErrExit "Could not edit database"
