...
def plot_learning_curves(names_, classifiers_, suffix_, folder, x_data, y_data, npoints):
    ...
    for ind, (name, clf) in enumerate(zip(names_, classifiers_), start=1):
        ...
        ax.set_xlabel("Training set size", fontsize=40)
        ax.set_ylabel("Minimum squared error", fontsize=40)
        ax.set_title(f"Learning curve XGBoost", fontsize=40)
        ax.legend(loc="best", frameon=False, fontsize=25)
...
def plot_model_pred(names, classifiers, suffix, x_train, y_train, x_test, y_test, folder, class_labels, bins=50):
    for name, clf in zip(names, classifiers):
        ...
        for cls_hyp, label_hyp in enumerate(class_labels):
            ...
            plt.yscale("log")
            figure.text(0.35, 0.8, "This Thesis", fontsize=40)
